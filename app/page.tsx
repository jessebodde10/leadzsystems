...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#111418] text-white overflow-x-hidden">
      ...
      {/* ── SECTOR-PREVIEW (werkbon + planning) — verborgen op mobiel ── */}
      <section className="hidden md:block py-16 md:py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
      ...
      {/* ── MOBIELE FLOATING KNOPPEN (bel links, WhatsApp rechts) ── */}
      <a
        href={`tel:${SITE.phoneIntl}`}
        aria-label={`Bel ${SITE.phone}`}
        className="md:hidden fixed bottom-6 left-6 z-50 w-14 h-14 rounded-full bg-gradient-to-r from-orange-500 to-orange-600 shadow-lg shadow-orange-500/30 flex items-center justify-center"
      >
        <span className="sr-only">Bel {SITE.phone}</span>
        <svg viewBox="0 0 24 24" fill="white" className="w-6 h-6" aria-hidden="true">
          <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.24.2 2.45.57 3.57.11.35.03.74-.24 1.02l-2.21 2.18z" />
        </svg>
      </a>
      <a
        href={`https://wa.me/${SITE.whatsapp}`}
        ...
        className="md:hidden fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#25D366] shadow-lg shadow-black/30 flex items-center justify-center"
      >
